...
public class DVAComponentes extends JFrame {
	...
	public DVAComponentes(JFrame TelaInicial) {
		...
		tblComponentes.setModel(ModeloDados);
		PainelTabela.setViewportView(tblComponentes);
		...
		btnExcluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(TelaPrincipal.ComponentesCadastrados.size() > 0) {
					if(tblComponentes.getSelectedRowCount() > 0) {
		...
		btnAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(TelaPrincipal.ComponentesCadastrados.size() > 0) {
					int Linha = tblComponentes.getSelectedRow();
					DefaultTableModel ModeloDados2 = (DefaultTableModel) tblComponentes.getModel();
					try {
					...
					lblMensagemBotao.setText("Adicione um componente para alterar!");
				}
			}
		});
				...
				try {
		            ...
		            if(ValorPreco >= 0) {
						...
						for (Composicao i : TelaPrincipal.ComposicoesCadastradas) {
							if(i.getIDComponente() == Codigo) {
								i.setNomeComponente(txtAltNome.getText());
							}
						}
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Componente i : TelaPrincipal.ComponentesCadastrados) {
					if (i.getID() == Codigo) {
						...
						TelaPrincipal.ComponentesCadastrados.remove(i);
						lblMensagemBotao.setText("Exclusao concluida!");
						break;
					}
				}
				TelaPrincipal.ComposicoesCadastradas.removeIf(t -> t.getIDComponente() == Codigo);
